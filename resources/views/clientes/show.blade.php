...
<div class="container-fluid">
    <div class="row">
        <div class="col-xl-12"><h1 class="page-header"><a href="{{ route('clientes.index') }}"><i class="fa fa-fw fa-angle-double-left"></i></a>
            @yield('title')<small>{{ $cliente->full_name }}</small></h1>
        </div>
        <!-- /.col-xl-12 -->
    </div>
    <!-- /.row -->
...
<div class="panel panel-default">
    <!--<div class="panel-heading">
        <h1 class="page-header">@yield('title')<h2> {{ $cliente->full_name }}</h2></h1>
        <ul>
    ...
    <div class="panel-body">
                ...
                <div class="card">
                    ...
                    <div class="card-body bg-white text-dark">
                        <!-- Nav tabs -->
                        <ul class="h3 nav nav-tabs">
                            <li class="nav-item">
                                <!--<a href="#" class="add-Consultamodal"><i class="fa fa-plus-circle fa-fw"></i></a>-->
                                <a href="#" class="add_evento-modal"
                                    data-toggle='modal' 
                                    data-target='#add_evento'
                                    date-id_cliente="$cliente->id" 
                                    data-start="{{ 'start' }}"
                                    data-fecha="{{ 'fecha' }}"
                                    ><i class="fa fa-plus-circle fa-fw"></i>Añadir Consulta</a>
                            </li>
                            <li class="nav-item active"><a href="#home" data-toggle="tab" class="nav-link active" aria-expanded="true">Histórico
                                <span class="badge badge-primary badge-pill">{{ $cliente->consultas()->count() }}</span></a></li>
                            ...
                            <li class="nav-item" style="float:right"><a href="#imprimir" data-toggle="tab" class="nav-link"><i class="fa fa-print fa-fw"></i></a></li>
                        </ul>
                        <!-- Tab panes -->
                                    ...
                                    <tbody>                                        
                                        @foreach($consultas as $consulta)                                            
                                            <tr class="item{{$consulta->id}} 
                                                @if($consulta->variacion>0) warning
                                                @elseif($consulta->comentario=='peso esperado') info 
                                                @elseif($consulta->comentario=='consulta inicial') danger
                                                @elseif(!$consulta->asistio) success @endif">                                
    ...
    <script>
        $(function() {                                               
            Morris.Line({
                ...
                // the chart.
                data: [                    
                    //{"fecha": "{{ \Carbon\Carbon::createFromTimeString($cliente->created_at)->format('Y-m-d') }}", "inicial": {{ $cliente->peso_inicial }} },
                    //{"fecha": "{{ TODAY()->format('Y-m-d') }}", "inicial": {{ $cliente->peso_inicial }} }, 
                    //{"fecha": "{{ \Carbon\Carbon::createFromTimeString($cliente->created_at)->format('Y-m-d') }}", "saludable": {{ $cliente->peso_saludable }} },    
                    //{"fecha": "{{ TODAY()->format('Y-m-d') }}", "saludable": {{ $cliente->peso_saludable }} },
                    {"fecha": "{{$cliente->consultas()->orderbydesc('fecha')->first()->fecha->format('Y-m-d')}}",'inicial': {{ $cliente->peso_inicial }} },
                    {"fecha": "{{$cliente->consultas()->orderby('fecha')->first()->fecha->format('Y-m-d')}}",'inicial': {{ $cliente->peso_inicial }} },
                    {"fecha": "{{$cliente->consultas()->orderbydesc('fecha')->first()->fecha->format('Y-m-d')}}",'saludable': {{ $cliente->peso_saludable }} },
                    {"fecha": "{{$cliente->consultas()->orderby('fecha')->first()->fecha->format('Y-m-d')}}",'saludable': {{ $cliente->peso_saludable }} },
                    @foreach($consultas as $consultas())
                        ...
                        {"fecha": "{{ $consulta->fecha->format('Y-m-d') }}", "peso": {{ $consulta->peso }}, "variacion": {{ $consulta->variacion }} },
                    @endforeach
